...
west_params <- list(cutoff_values = c(3, 5, 7, 10, 15, 20),
                    max_edge_in = c(15, 20, 35, 50, 75, 100),
                    max_edge_out = rep(100, 6))

east_params <- list(cutoff_values = c(3, 5, 7, 10, 15, 20),
                    max_edge_in = c(15, 20, 35, 50, 75, 100),
                    max_edge_out = rep(100, 6))
...
# ------------------------------------------------------------------------------#
source(here::here("04_MODEL/functions_all_mesh_configs/cross_validation_repeated_random.R"))
repeated_randomCV_by_region <- list()

if (isTRUE(West_English_Channel)) {
  
  repeated_randomCV_by_region$west <- repeated_random_CV(
    converged_models = converged_models, meshes_by_region = meshes_by_region,
    region_name = "west", n_repeats = 10, k_folds = 10, seed = 123)
  
}

if (isTRUE(East_English_Channel)) {
repeated_randomCV_by_region$east <- repeated_random_CV(
  converged_models = converged_models, meshes_by_region = meshes_by_region,
  region_name = "east", n_repeats = 10, k_folds = 10, seed = 123)
}
