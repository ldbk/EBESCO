...
create_meshes_configurations <- function(region_name,
  ...
  for (i in seq_along(cutoff_values)) {
    ...
    message(glue::glue(
      "{sp_scientific} | {region_name} | Testing cutoff = {cutoff} ; max.edge = ({max_edge[1]}, {max_edge[2]})"
    ))
    ...
    plot_mesh <- ggplot() +
      ...
      labs(x="", y="", title = paste0("Mesh with cutoff = ", round(cutoff,1), "km"))


    # -----------------------------------------------------------------------#
    # Return useful outputs for this configuration
    # -----------------------------------------------------------------------#
