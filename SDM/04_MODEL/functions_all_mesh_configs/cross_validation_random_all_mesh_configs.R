...
random_CV <- function(converged_models, meshes_by_region, region_name) {
  ...
  for (response_name in names(models_by_response)) {
    ...
    for (family_name in names(models_by_family)) {
      ...
      message("Cross-validation: ", region_name, " - ", mesh_name, " - ", response_name, " - ", family_name)
      ...
      cv_scores[[mesh_name]][[response_name]][[family_name]] <- tibble::tibble(
        region = region_name,
        mesh = mesh_name,
