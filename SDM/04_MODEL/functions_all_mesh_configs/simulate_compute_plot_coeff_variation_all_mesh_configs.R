...
simulate_cv <- function(model, grid, response) {
  ...
  if (response == "totalWeightKg") {
    grid_pred$logSweptAreaKm2 <- log(res_km * res_km)
    offset_pred <- grid_pred$logSweptAreaKm2
  } else {
    offset_pred <- NULL
  }
  ...
  grid_pred$cv <- apply(simulations, 1, function(x) sd(x) / mean(x))
...
for (region in names(converged_models)) {
  ...
  sim_cv_all[[region]] <- list()
  
  for (mesh_name in names(converged_models[[region]])) {
    
    sim_cv_all[[region]][[mesh_name]] <- list()
    
    for (response in names(converged_models[[region]][[mesh_name]])) {
      
      sim_cv_all[[region]][[mesh_name]][[response]] <- list()
      
      for (model_name in names(converged_models[[region]][[mesh_name]][[response]])) {
        
        mod <- converged_models[[region]][[mesh_name]][[response]][[model_name]]
        
        pred_cv <- simulate_cv(model = mod, grid = grid_region, response = response)
        
        sim_cv_all[[region]][[mesh_name]][[response]][[model_name]] <- pred_cv
      }
    }
  }
}
...
# CV common limits
cv_range_global <- range(
  unlist(
    lapply(sim_cv_all, function(region_list) {
      lapply(region_list, function(mesh_list) {
        lapply(mesh_list, function(response_list) {
...
for (region in names(sim_cv_all)) {
  ...
  first_mesh <- names(sim_cv_all[[region]])[1]
  first_response <- names(sim_cv_all[[region]][[first_mesh]])[1]
  first_model <- names(sim_cv_all[[region]][[first_mesh]][[first_response]])[1]
  
  grid_region <- sim_cv_all[[region]][[first_mesh]][[first_response]][[first_model]]
...
for (region in names(sim_cv_all)) {
  ...
  plots_cv_all[[region]] <- list()
  
  for (mesh_name in names(sim_cv_all[[region]])) {
    
    plots_cv_all[[region]][[mesh_name]] <- list()
    
    for (response in names(sim_cv_all[[region]][[mesh_name]])) {
      
      plots_cv_all[[region]][[mesh_name]][[response]] <- list()
      
      for (model_name in names(sim_cv_all[[region]][[mesh_name]][[response]])) {
        
        df <- as.data.frame(
          sim_cv_all[[region]][[mesh_name]][[response]][[model_name]]
        )
        
        plots_cv_all[[region]][[mesh_name]][[response]][[model_name]] <-
          plot_cv_map(
            data = df,
            land_obj = land_obj,
            subtitle = paste(region, mesh_name, response, model_name, "CV", sep = " - "),
