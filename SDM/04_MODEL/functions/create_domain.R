...
# Save
writeVector(BaieSeine, here("01_DATA/shapefiles/BaieDeSeine/BaieDeSeine.shp"))
...
# Define a geographic bounding box to delimit the eastern part of the Channel
east_ext <- ext(-1.4263, 1.930905, 49.2, 51.166)

# Convert the extent into a polygon with the same CRS as the English Channel shapefile
east_box <- as.polygons(east_ext, crs = crs(English_Channel))

# Clip the English Channel using the polygon to keep only its eastern part
east_English_Channel <- intersect(English_Channel, east_box)
...
# Save
writeVector(east_English_Channel, here("01_DATA/shapefiles/split_English_Channel/east_English_Channel.shp"))
...
west_ext <- ext(-5.709, -1.4263, 48.4, 51.166)

# Convert the extent into a polygon with the same CRS as the English Channel shapefile
west_box <- as.polygons(west_ext, crs = crs(English_Channel))

# Clip the English Channel using the polygon to keep only its western part
west_English_Channel <- intersect(English_Channel, west_box)

# Plot the spatial result to visually confirm the extracted marine area
as.data.frame(west_English_Channel)
plot(west_English_Channel, col = "grey80", main = "western_English_Channel")

# Save
writeVector(west_English_Channel, here("01_DATA/shapefiles/split_English_Channel/west_English_Channel.shp"))
