...
create_meshes_configurations <- function(region_name,
  ...
  use_max_edge <- !is.null(max_edge_in) && !is.null(max_edge_out)
  
  if (use_max_edge) {
    if (length(max_edge_in) != length(cutoff_values) ||
        length(max_edge_out) != length(cutoff_values)) {
      stop("max_edge_in and max_edge_out must have the same length as cutoff_values")
    }
  }
  ...
  for (i in seq_along(cutoff_values)) {
    ...
    message(glue::glue(
      "{sp_scientific} | {region_name} | Testing cutoff = {cutoff} ; boundary = {boundary} ; barrier = {add_barrier}"
    ))
    ...
    config_result <- tryCatch({
      ...
      # Add max.edge only if provided
      if (use_max_edge) {
        mesh_arguments$max.edge <- max_edge
      }
      ...
      if ((sp_scientific == "Trachurus trachurus") && (region_name == "west")) {
        ...
        family_type = delta_lognormal(link1 = "logit", link2 = "log")
      }
