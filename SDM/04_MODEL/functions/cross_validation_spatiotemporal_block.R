...
spatiotemp_blocked_CV <- function(sanity_output, region, data, mesh) {
...
if (isTRUE(East_English_Channel)) {
  blockedCV_by_region$east <-  spatiotemp_blocked_CV(sanity_output  = sanity_by_region$east,
                                                            region = "east",
                                                            data = data_CGFS_east,
...
if (isTRUE(West_English_Channel)) {
  blockedCV_by_region$west <- spatiotemp_blocked_CV(sanity_output  = sanity_by_region$west, 
                                                           region = "west",
                                                           data = data_CGFS_west,
...
if (isTRUE(East_English_Channel)) {
  ...
  conv_summary_east <- imap_dfr(blockedCV_by_region$east$cv_outputs,
...
if (isTRUE(West_English_Channel)) {
  ...
  conv_summary_west <- imap_dfr(blockedCV_by_region$west$cv_outputs,
                                function(resp_list, response) {
                                  imap_dfr(resp_list, function(cv, family) {
                                    tibble(response = response,
                                           family = family,
